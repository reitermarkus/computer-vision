...
int main(int argc, char **argv) {
  ...
  vector<double> theta = {0, M_PI / 4, M_PI / 2, 3 * M_PI / 4}; // 4 orientations for the size of the filter
  int n = 15;
  Size gsize = {n, n};
  ...
  for (int i = 1; i <= 4; i++) {
    ...
    Mat gkernel = getGaborKernel(gsize, 10, theta[i], 10.0, 1.0, 0.0, CV_32F); // apply Gabor
    ...
    filter2D(image, respf, CV_32F, gkernel, Point(-1,-1));
    ...
    moveWindow("Image " + to_string(i), (i - 1) * 336, 448);
